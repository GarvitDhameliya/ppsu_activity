...
const Export = (props) => {
    ...
    const exportpdf = () => {
        ...
        props.data[0].map((data, ind) => {
            report_data.push([ind + 1, data.student.enrollmentNo, `${data.heading.categoryId}_${data.heading.categoryName}`, data.class.betch, data.class.name, data.student.totalPoint, data.status, data.publishDate.split('T')[0]])
        })
        ...
        autoTable(doc, {
            ...
            showHead: 'everyPage',
            head: [['index', 'Enrollment No', 'Activity Name', 'Year', 'Class', 'TotalPoint', 'Status', 'Publish Date']],
            body: report_data
        })
        ...
        // Name of PDF
        doc.save(`report_${instAdminReducer.adminProfile.school.name}.pdf`)
    }
    ...
    const exportCsv = () => {
        let csvRow = []
        let a = [['', 'Enrollment No', 'Activity Name', 'Year', 'Class', 'TotalPoint', 'Status', 'Publish Date']]

        props.data[0].map((data, ind) => {
            a.push([data.student.enrollmentNo, data.student.name, data.class.betch, `${data.heading.categoryId}_${data.heading.categoryName}`, data.student.totalPoint, data.status, data.publishDate.split('T')[0]])
        })
        ...
        anchor.download = `report_${instAdminReducer.adminProfile.school.name}.csv`
