...
const Export = (props) => {
    ...
    const exportpdf = () => {
        ...
        props.data[0].map((data, ind) => {
            report_data.push([ind + 1, data.student.enrollmentNo, data.student.name, `${data.heading.categoryId}_${data.heading.categoryName}`, data.class.name, data.class.betch, data.publishDate.split('T')[0], data.status])
        })
        ...
        autoTable(doc, {
            ...
            showHead: 'everyPage',
            head: [['index', 'Enrollment No', 'Name', 'Activity Name', 'Year', 'Class', 'Publish Date', 'Status']],
            body: report_data
        })
        ...
        // Name of PDF
        doc.save(`report_${apcReducer.apcProfile.school.name}.pdf`)
    }
    ...
    const exportCsv = () => {
        let csvRow = []
        let a = [['', 'Enrollment No', 'Name', 'Activity Name', 'Year', 'Class', 'Publish Date', 'Status']]

        props.data[0].map((data, ind) => {
            a.push([data.student.enrollmentNo, data.student.name, `${data.heading.categoryId}_${data.heading.categoryName}`, data.class.name, data.class.betch, data.publishDate.split('T')[0], data.status])
        })
        ...
        anchor.download = `report_${apcReducer.apcProfile.school.name}.csv`
